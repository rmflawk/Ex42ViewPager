package com.youngstudio.ex42viewpager;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //page를 조금이라도 움직일때마다
        //자동으로 발동하는 메소드를 보유한 리스너를 추가
        pager.setPageTransformer(false, new ViewPager.PageTransformer() {
            @Override
            public void transformPage(@NonNull View page, float position) {
                // 현재번호 0번 0.0시작 다음번호 1번 1.0시작 -0.1씩 감소 자동으로 3번씩호출
                page.setRotationY(position*90);
                page.setScaleX( (1- Math.abs(position))/2+0.5f); // 무조건 양수로
                page.setScaleY( (1- Math.abs(position))/2+0.5f); // 무조건 양수로
